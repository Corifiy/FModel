...
public class UMaterial : UMaterialInterface
{
    ...
    public override void Deserialize(FAssetArchive Ar, long validPos)
    {
        ...
        {
            // The object-version check above is the real, authoritative signal for whether this
            // archive serializes inline shader maps at all (FMaterialShaderMapLegacy format) -
            // Ar.Ver comes directly from the package's own header. A Game>=GAME_UE4_23 check used
            // to additionally gate this, on the assumption that pre-4.23 titles wouldn't have this
            // format yet - but Epic's internal Fortnite branch (a GAME_UE4_19-tagged build, per
            // EGame) already reports Ar.Ver over this threshold, meaning their branch backported the
            // inline-shader-map change well ahead of the public 4.23 release; the extra Game gate
            // was silently skipping real, present, correctly-versioned shader map data for it.
            // DeserializeInlineShaderMaps' own internal validation (FMaterialResourceProxyReader's
            // bCooked trailing-flag check, SkipToDebugDescription's anchor scan) already throws
            // cleanly if the format genuinely doesn't match some other title's fork, so removing
            // this redundant gate doesn't trade away safety - the existing catch below still reverts
            // to the same "don't even attempt it" behavior on failure.
            if (Ar.Owner.Provider.ReadShaderMaps)
            {
                ...
                catch (Exception e)
                {
                    ...
                    // FMaterialResourceProxyReader.CreatePassthrough for why this exists at all. Clear
                    // whatever partial resources the failed first attempt already added (it adds
                    // incrementally per-resource, so a failure partway through leaves real entries
                    // behind) before retrying from the saved position.
                    ...
                    try
                    {
                        DeserializeInlineShaderMaps(Ar, LoadedMaterialResources, usePassthrough: true);
                    }
                    catch (Exception e2)
                    {
                ...
                if (succeeded)
                {
                    ...
                    {
                        Log.Error(e3, "Failed to deserialize inline shader maps.");
                        succeeded = false;
                    }
                }
                ...
                if (!succeeded)
                {
                    // Recover to validPos, not the mid-parse "saved" position: a genuine format
                    // mismatch means everything from "saved" onward is unreliable, and validPos is
                    // where the caller's own property-iteration logic expects this block to end
                    // regardless of whether it was parsed - the same recovery the "don't even
                    // attempt it" branch below already relies on.
                    LoadedMaterialResources.Clear();
