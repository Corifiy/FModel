...
public class FMaterial
{
    ...
    /// <summary>Shader map in the legacy (pre-FMemoryImage, &lt; 4.25) cooked format.</summary>
    public FMaterialShaderMapLegacy? LoadedShaderMapLegacy;

    /// <summary>
    /// <paramref name="expectedFriendlyName"/>, when known, is the owning UMaterialInterface's own
    /// short Name - confirmed against engine source (Materials/MaterialInstanceSupport.h:89,
    /// FMaterialInstanceResource::GetFriendlyName: "return Owner->GetName();") as exactly what gets
    /// written into FMaterialShaderMap::FriendlyName during cooking. Only consulted by the very-legacy
    /// (pre-proxy-reader) anchor scan in FMaterialShaderMapIdLegacy, where it turns a generic "looks
    /// like a plausible short ASCII string" heuristic into an exact-match search - eliminating the
    /// real false-positive risk of the generic heuristic matching stray bytes inside that era's
    /// unmodeled FStaticParameterSet/dependency-array fields before reaching the real FriendlyName.
    /// </summary>
    public void DeserializeInlineShaderMap(FMaterialResourceProxyReader Ar, string? expectedFriendlyName = null)
    {
        ...
        if (bValid)
        {
            ...
            {
                // pre-FMemoryImage format (FMaterialShaderMap::Serialize legacy path).
                // The format is unversioned (DDC-keyed in the engine), so branches cut before
                // the 4.23 virtual-texture merge serialize differently: parse-validate-retry
                // across the known byte-layout profiles, keeping the last one that worked.
                var mapStart = Ar.Position;
                // UE4_19 is only ever attempted for the passthrough (pre-proxy-reader) reader - see
                // FMaterialResourceProxyReader.CreatePassthrough. Normal-format archives (4.22+,
                // including every already-working title) never construct a passthrough reader, so
                // this never changes their profile list.
                var profiles = Ar.IsPassthrough
                    ? new[] { ELegacyShaderMapProfile.UE4_19, ELegacyShaderMapProfile.PreVirtualTexture, ELegacyShaderMapProfile.UE4_23 }
                    : Ar.LegacyProfile == ELegacyShaderMapProfile.UE4_23
                        ? new[] { ELegacyShaderMapProfile.UE4_23, ELegacyShaderMapProfile.PreVirtualTexture }
                        : new[] { ELegacyShaderMapProfile.PreVirtualTexture, ELegacyShaderMapProfile.UE4_23 };
                Exception? lastError = null;
                foreach (var profile in profiles)
                {
                    Ar.Position = mapStart;
                    Ar.LegacyProfile = profile;
                    try
                    {
                        var map = new FMaterialShaderMapLegacy();
                        if (map.Deserialize(Ar, expectedFriendlyName))
                        {
                            LoadedShaderMapLegacy = map;
                            return;
                        }
                        Log.Verbose("Legacy shader map rejected by validation under profile {0}", profile);
                    }
                    catch (Exception e) when (e is not OutOfMemoryException)
                    {
                        lastError = e;
                        Log.Verbose("Legacy shader map failed under profile {0}: {1}", profile, e.Message);
                    }
                }
                throw lastError ?? new ParserException(Ar, "Legacy shader map did not validate under any known byte-layout profile");
            }
...
public class FMaterialShaderMapId
{
    ...
    public FMaterialShaderMapId(FArchive Ar)
    {
        ...
        if (!bIsLegacyPackage)
        {
            ...
            if (Ar.Game is EGame.GAME_ArenaBreakoutInfinite or EGame.GAME_ArenaBreakoutMobile) Ar.Position += 4;
